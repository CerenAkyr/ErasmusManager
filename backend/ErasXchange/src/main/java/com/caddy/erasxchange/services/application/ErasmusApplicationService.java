...
public class ErasmusApplicationService extends ApplicationService<ErasmusApplication, ErasmusApplicationRepository> {
     ...
     */
    public List<FullErasmusApplicationDto> getAll() {
        return erasmusMapper.toFullDtoList(repository.findAll());
    }
    ...
    public ErasmusApplicationDto getApplication(Integer id) {
        Student student = studentService.findByBilkentId(id);
        return erasmusMapper.toDto(student.getErasmusApplication());
    }
     ...
     */
    public AppStatus getStudentAppStatus(Integer bilkentId) {
        Student student = studentService.findByBilkentId(bilkentId);
     ...
     */
    @Transactional
    public void startPlacements(Department department) throws MessagingException, UnsupportedEncodingException {
    ...
    public void checkApplications(Department department) {
        ...
        if (placementStatus == PlacementStatus.APPS_CORRECT ||
            ...
            List<ErasmusApplication> wrongApps =  getWrongApplications(department);
            if (wrongApps.size() == 0) {
    ...
    private List<ErasmusApplication> getWrongApplications(Department department) {
    ...
    public List<FullErasmusApplicationDto> getWrongAppsByDepartment( Department department) {
        List<ErasmusApplication> wrongApps = getWrongApplications(department);

        return erasmusMapper.toFullDtoList(wrongApps);

    }

    public void activateApplications(Department department) {
        checkApplications(department);
    }
}
