...
@Mapper(unmappedTargetPolicy = ReportingPolicy.IGNORE, componentModel = "spring",
        uses =
                {ErasmusUniversityService.class, StudentService.class, ErasmusUniversityMapper.class, StudentMapper.class})
...
//    ErasmusApplication toEntity1(FullErasmusApplicationDto fullErasmusApplicationDto);

    FullErasmusApplicationDto toFullDto(ErasmusApplication erasmusApplication);
    List<FullErasmusApplicationDto> toFullDtoList(List<ErasmusApplication> erasmusApplications);
    @BeanMapping(nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
    ErasmusApplication partialUpdate1(FullErasmusApplicationDto fullErasmusApplicationDto, @MappingTarget ErasmusApplication erasmusApplication);
    /*
